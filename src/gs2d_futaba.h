...
class Futaba : public CommandHandler<SerialClass, bufferSize, commandSize>, public Driver
{
  private:
    ...
    void dataReceivedEvent(uint8_t *data, uint8_t length, uint8_t status)
    {
        ...
        do
        {
            ...
            // チェックサム確認
            if (data[length - 1] != calculateCheckSum(data, length))
            {
        ...
        if (this->currentCommand.responseProcess)
        {
            if (this->currentCommand.callback)
            {
                CallbackEventArgs e(data[2], this->errorBits,
    ...
    void setFunction(uint8_t id, uint8_t address, uint32_t param = 0, uint8_t length = 0,
                     ResponseProcess responseProcess = 0, CallbackType callback = 0,
                     uint8_t flag = 0, uint8_t count = 1)
    {
    ...
    EventDataType getFunction(uint8_t id, uint8_t address, uint8_t flag, uint8_t length,
                              ResponseProcess responseProcess = 0, CallbackType callback = 0)
    {
    ...
    void setFunctionBurstWrite(uint8_t id, uint8_t address, uint8_t flag, uint8_t length,
                               uint8_t *idList, uint32_t *dataList, uint8_t count,
                               ResponseProcess responseProcess = 0, CallbackType callback = 0)
    {
    ...
    {
        return EventDataType((gFloat)(-(int16_t)data / 10.0));
    }
    static EventDataType targetTimeProcess(int32_t data)
    {
        return EventDataType((gFloat)(data / 100.0));
    }
    static EventDataType speedCallback(int32_t data)
    {
        return EventDataType((int32_t)(int16_t)data);
    }
    static EventDataType baudrateProcess(int32_t data)
    {
        int32_t baudrateList[10]{9600,  14400, 19200,  28800,  38400,
                                 57600, 76800, 115200, 153600, 230400};
        if (data > 9)
            return EventDataType((int32_t)0);
  ...
  public:
    ...
    gFloat readCurrentPosition(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (gFloat)getFunction(id, Address::CurrentPosition, 0x0F, 2, positionProcess,
                                   callback);
    }
    ...
    void writeLimitCWPosition(uint8_t id, gFloat limitPosition)
    {
        ...
        if (limitPosition < -150 || limitPosition > 0)
        {
            badInput();
            return;
        }
    ...
    void burstWriteTargetPositions(uint8_t *idList, gFloat *positionList, uint8_t count)
    {
        ...
        setFunctionBurstWrite(0, Address::TargetPosition, 0, 3, idList, positionListInt, count, 0,
                              0);
