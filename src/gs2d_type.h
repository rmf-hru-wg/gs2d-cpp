...
class EventDataType
{
  ...
  public:
    EventDataType(gFloat data)
    {
        intData = (int32_t)data;
        floatData = data;
        flag = true;
    }
    EventDataType(int32_t data)
    {
        intData = data;
        floatData = (gFloat)data;
        flag = false;
    }
    EventDataType()
    {
...
class CallbackEventArgs
{
  public:
    ...
    CallbackEventArgs(uint8_t id, uint8_t status, EventDataType data) : id(id), status(status)
    {
        this->data = data;
    }
...
// gs2d用サーキュラバッファクラス
template <class T, unsigned char bufferSize = 50> class CircularBuffer
{
  private:
    T buffer[bufferSize];
...
template <class T> class Gs2dType
{
  private:
    volatile T variable;
