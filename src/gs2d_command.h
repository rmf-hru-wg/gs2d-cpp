...
/* Variables -----------------------------------------------------------------*/
// コマンドバッファの型
template <unsigned int commandSize = 50> class CommandBufferType
{
  public:
    ...
    CommandBufferType(uint8_t *data, uint8_t length, uint8_t count = 1,
                      ResponseProcess response = 0, CallbackType callback = 0)
    {
