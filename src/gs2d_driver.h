...
class Driver
{
  protected:
    ...
    void badInput(void) { errorBits |= BadInputError; }
  ...
  public:
    ...
    // ------------------------------------------------------------------------------------------
    // General
    virtual uint32_t readMemory(uint8_t id, uint16_t address, uint8_t length,
    ...
    // Burst Function
    virtual void burstReadMemory(uint8_t *idList, uint8_t count, uint16_t address, uint8_t length,
                                 CallbackType callback) = 0;
    virtual void burstWriteMemory(uint8_t *idList, uint32_t *dataList, uint8_t count,
                                  uint16_t address, uint8_t length) = 0;

    // Burst Function(Position)
    virtual void burstReadPositions(uint8_t *idList, uint8_t count, CallbackType callback) = 0;
    virtual void burstWriteTargetPositions(uint8_t *idList, gFloat *positionList,
                                           uint8_t count) = 0;
};
} // namespace gs2d
