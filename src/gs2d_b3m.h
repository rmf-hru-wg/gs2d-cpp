...
class B3M : public CommandHandler<SerialClass, bufferSize, commandSize>, public Driver
{
  private:
    ...
    void dataReceivedEvent(uint8_t *data, uint8_t length, uint8_t status)
    {
        ...
        do
        {
            ...
            // チェックサム確認
            if (calculateCheckSum(data, length) != data[length - 1])
            {
                this->errorBits |= ResponseError;
                break;
            }
        ...
        if (this->currentCommand.responseProcess)
        {
            if (this->currentCommand.callback)
            {
                CallbackEventArgs e(data[3], this->errorBits,
    ...
    {
        return EventDataType((int32_t)((data >> 8) & 0xFF));
    }
    static EventDataType torqueProcess(int32_t data)
    {
        return EventDataType((int32_t)((data & 0b10) ? 0 : 1));
    }
    static EventDataType temperatureProcess(int32_t data)
    {
        return EventDataType((int32_t)(data / 100));
    }
    ...
    static EventDataType positionProcess(int32_t data)
    {
        return EventDataType((gFloat)(-(int16_t)data / 100.0));
    }
    static EventDataType deadbandProcess(int32_t data)
    {
        return EventDataType((gFloat)(data / 100.0));
    }
    ...
    static EventDataType positionLimitProcess(int32_t data)
    {
        return EventDataType((gFloat)(-(int16_t)data / 100.0));
    }
  ...
  public:
    ...
    void writePGain(uint8_t id, uint32_t gain)
    {
        ...
        uint8_t param[6]{gain & 0xFF,         (gain >> 8) & 0xFF, (gain >> 16) & 0xFF,
                         (gain >> 24) & 0xFF, Address::PGain,     1};
    ...
    void writeBaudrate(uint8_t id, uint32_t baudrate)
    {
        ...
        uint8_t param[6]{baudrate & 0xFF,         (baudrate >> 8) & 0xFF, (baudrate >> 16) & 0xFF,
                         (baudrate >> 24) & 0xFF, Address::Baudrate,      1};
    ...
    gFloat readLimitCWPosition(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (gFloat)getFunction(id, Instructions::Read, param, 2, positionLimitProcess,
                                   callback);
    }
                          ...
                          uint8_t length)
    {
        ...
        getFunction(idList[0], Instructions::Write, param, (length + 1) * (count) + 1, 0, 0,
                    (count == 1) ? 1 : 0);
        delete[] param;
    }
    ...
    void burstWriteTargetPositions(uint8_t *idList, gFloat *positionList, uint8_t count)
    {
        ...
        getFunction(idList[0], Instructions::Write, param, 3 * (count) + 1, 0, 0,
                    (count == 1) ? 1 : 0);
        delete[] param;
    }
};
