...
class RobotisP20 : public CommandHandler<SerialClass, bufferSize, commandSize>, public Driver
{
  private:
    ...
    void dataReceivedEvent(uint8_t *data, uint8_t length, uint8_t status)
    {
        ...
        do
        {
            ...
            if (data[8] != 0)
            {
                this->errorBits |= ProtocolError;
                break;
            }

            // CheckSum検証
            uint16_t crc = data[length - 2] + (data[length - 1] << 8);
            if (crc != gs2d::util::crc16::calculate(data, length - 2))
            {
                this->errorBits |= ResponseError;
                break;
            }
        ...
        if (this->currentCommand.responseProcess)
        {
            if (this->currentCommand.callback)
            {
                CallbackEventArgs e(data[4], this->errorBits,
                                    this->currentCommand.responseProcess(tmp));
                this->currentCommand.callback(e);
                if (!operatingMode)
                    isReceived.set(true);
            } else
    ...
    EventDataType getFunction(uint8_t id, uint8_t instruction, uint8_t *param = 0,
                              uint8_t length = 0, ResponseProcess responseProcess = 0,
                              CallbackType callback = 0, uint8_t count = 1)
    {
        // コマンドの長さを取得
        ...
        // 不必要なら空データを返す
        if (operatingMode && callback != 0 || count == 0)
            return EventDataType((int32_t)0);
    ...
    // �P�ʕϊ��֐�
    static EventDataType pingProcess(int32_t data)
    {
        return EventDataType((int32_t)(data & 0xFFFF));
    }
    static EventDataType readTorqueProcess(int32_t data)
    {
        return EventDataType((int32_t)((data) ? 1 : 0));
    }
    static EventDataType currentProcess(int32_t data)
    {
        return EventDataType((int32_t)(data * 2.69));
    }
    static EventDataType voltageProcess(int32_t data)
    {
        return EventDataType((gFloat)(data / 10.0));
    }
    static EventDataType targetPositionProcess(int32_t data)
    {
        return EventDataType((gFloat)(data * 360.0 / 4096.0 - 180.0));
    }
    static EventDataType offsetProcess(int32_t data)
    {
        return EventDataType((gFloat)(data * 0.088));
    }
    static EventDataType targetTimeProcess(int32_t data)
    {
        return EventDataType((gFloat)(data / 1000.0));
    }
    static EventDataType speedProcess(int32_t data)
    {
        return EventDataType((gFloat)(data * 0.229 * 6));
    }
  ...
  public:
    ...
    uint8_t readTorqueEnable(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (int32_t)getFunction(id, Instructions::Read, param, length, readTorqueProcess,
                                    callback);
    }
    ...
    int32_t readCurrent(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (int32_t)getFunction(id, Instructions::Read, param, length, currentProcess,
                                    callback);
    }
    ...
    gFloat readTargetPosition(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (gFloat)getFunction(id, Instructions::Read, param, length, targetPositionProcess,
                                   callback);
    }
    ...
    void writeTargetPosition(uint8_t id, gFloat position)
    {
        ...
        uint8_t param[6];
        uint8_t length = generateParameters(
            Address::GoalPosition, (uint32_t)((position + 180.0) * 4096.0 / 360.0), 4, param);
    ...
    gFloat readTargetTime(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (gFloat)getFunction(id, Instructions::Read, param, length, targetTimeProcess,
                                   callback);
    }
    void writeTargetTime(uint8_t id, gFloat targetTime)
    {
        ...
        uint8_t param[6];
        uint8_t length =
            generateParameters(Address::ProfileVelocity, (uint32_t)targetTime, 4, param);
    ...
    void writeAccelTime(uint8_t id, gFloat accelTime)
    {
        ...
        uint8_t param[6];
        uint8_t length =
            generateParameters(Address::ProfileAcceleration, (uint32_t)accelTime, 4, param);
    ...
    uint32_t readBaudrate(uint8_t id, CallbackType callback = 0)
    {
        ...
        return (int32_t)getFunction(id, Instructions::Read, param, length, baudrateProcess,
                                    callback);
    }
    ...
    void writeLimitCWPosition(uint8_t id, gFloat limitPosition)
    {
        ...
        if (limitPosition > 180.0)
            limitPosition = 180.0;
        else if (limitPosition < -180.0)
            limitPosition = -180.0;

        uint8_t param[6];
        uint8_t length =
            generateParameters(Address::MinPositionLimit,
                               (uint32_t)((limitPosition + 180.0) * 4096.0 / 360.0), 4, param);
    ...
    void writeLimitCCWPosition(uint8_t id, gFloat limitPosition)
    {
        ...
        uint8_t param[6];
        uint8_t length =
            generateParameters(Address::MaxPositionLimit,
                               (uint32_t)((limitPosition + 180.0) * 4096.0 / 360.0), 4, param);
    ...
    void writeLimitTemperature(uint8_t id, uint32_t temperature)
    {
        ...
        uint8_t param[6];
        uint8_t length =
            generateParameters(Address::TemperatureLimit, (uint32_t)temperature, 1, param);
    ...
    void writeLimitCurrent(uint8_t id, uint32_t current)
    {
        ...
        uint8_t param[6];
        uint8_t length =
            generateParameters(Address::CurrentLimit, (uint32_t)(current / 2.69), 2, param);
    ...
    // Burst Function
    void burstReadMemory(uint8_t *idList, uint8_t count, uint16_t address, uint8_t length,
    ...
    void burstWriteMemory(uint8_t *idList, uint32_t *dataList, uint8_t count, uint16_t address,
                          uint8_t length)
    {
        ...
        getFunction(0xFE, Instructions::SyncWrite, param, 4 + count * (length + 1), 0,
                    defaultWriteCallback, 0);
        delete[] param;
    }
    ...
    void burstReadPositions(uint8_t *idList, uint8_t count, CallbackType callback)
    {
        ...
        getFunction(0xFE, Instructions::SyncRead, param, count + 4, targetPositionProcess, callback,
                    count);
        delete[] param;
    }
    void burstWriteTargetPositions(uint8_t *idList, gFloat *positionList, uint8_t count)
    {
        ...
        getFunction(0xFE, Instructions::SyncWrite, param, 4 + count * 5, 0, defaultWriteCallback,
                    0);
        delete[] dataList;
        delete[] param;
    }
