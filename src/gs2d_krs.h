...
class KRS : public CommandHandler<SerialClass, bufferSize, commandSize>, public Driver
{
  private:
    ...
    void addCommand(uint8_t *data, uint8_t length, ResponseProcess response, CallbackType callback,
                    KRSTarget target = {0, 0})
    {
        CommandBufferType<commandSize> command(data, length, 1, response, callback);
        this->commandStack.push(command);
        targetStack.push(target);
        if (this->isTrafficFree.get())
            sendCommand();
    }
    ...
    void dataReceivedEvent(uint8_t *data, uint8_t length, uint8_t status)
    {
            ...
            case 0:
                ...
                for (uint8_t t = 0; t < eepromBuf.size(); t++)
                {
                    if (eepromBuf[t].id == id)
                    {
                        ...
                        {
                            tmp += (eepromBuf[t].data[currentTarget.address + i]
                                    << (4 * (currentTarget.length - i - 1)));
                        }
                        break;
                    }
        ...
        if (this->currentCommand.responseProcess)
        {
            if (this->currentCommand.callback)
            {
                CallbackEventArgs e(data[0] & 0b11111, this->errorBits,
    ...
    EventDataType getFunction(uint8_t *command, uint8_t length, ResponseProcess responseProcess = 0,
                              CallbackType callback = 0, KRSTarget target = {0, 0})
    {
    ...
    {
        return EventDataType((int32_t)(100 - (data - 30) / 1.425));
    }
    static EventDataType currentProcess(int32_t data)
    {
        return EventDataType((int32_t)((data < 63) ? data * 100 : (data - 64) * 100));
    }
    static EventDataType positionProcess(int32_t data)
    {
        return EventDataType((gFloat)((7500 - data) / 29.629));
    }
    static EventDataType baudrateProcess(int32_t data)
    {
        ...
        {
        case 0x0A:
            return EventDataType((int32_t)(115200));
        case 0x01:
            return EventDataType((int32_t)(62000));
        case 0x00:
            return EventDataType((int32_t)(1250000));
        default:
            return EventDataType((int32_t)(0));
        }
    }
  ...
  public:
    ...
    void writeMemory(uint8_t id, uint16_t address, uint32_t data, uint8_t length)
    {
        ...
        if (!isRomDataAvailable(id))
        {
            this->errorBits |= SystemError;
            return;
        }
        if (address + length > 64)
        {
        ...
        for (uint8_t i = 0; i < length; i++)
            eepromData[address + i] = ((data >> (8 * i)) & 0xFF);

        uint8_t command[66];
        command[0] = (0b11000000 | id);
        command[1] = 0;
