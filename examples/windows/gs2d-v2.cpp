...
{
    std::cout << "Burst Read Result -> id:" << (int)arg.id << ", position:" << (gFloat)arg.data
              << std::endl;
}
...
int main()
{
    ...
    servo->writeOffset(id, 3.0);
    std::cout << "Offset Read : " << servo->readOffset(id) << std::endl;

    // Deadband
    std::cout << "Deadband Write : " << std::endl;
    servo->writeDeadband(id, 3.0);
    std::cout << "Deadband Read : " << servo->readDeadband(id) << std::endl;

    // P Gain
    std::cout << "PGain Write : " << std::endl;
    servo->writePGain(id, 800);
    std::cout << "PGain Read : " << servo->readPGain(id) << std::endl;

    // I Gain
    std::cout << "IGain Write : " << std::endl;
    servo->writeIGain(id, 400);
    std::cout << "IGain Read : " << servo->readIGain(id) << std::endl;

    // D Gain
    std::cout << "DGain Write : " << std::endl;
    servo->writeDGain(id, 300);
    std::cout << "DGain Read : " << servo->readDGain(id) << std::endl;

    // Max Torque
    std::cout << "MaxTorque Write : " << std::endl;
    servo->writeMaxTorque(id, 80);
    ...
    // ROM
    std::cout << "Save ROM : " << std::endl;
    servo->saveRom(id);
    std::cout << "Load ROM : " << std::endl;
    servo->loadRom(id);
    std::cout << "ResetMemory : " << std::endl;
    servo->resetMemory(id);

    Sleep(1000);

    // Baudrate
    std::cout << "Baudrate Write : " << std::endl;
    servo->writeBaudrate(id, 115200);
    std::cout << "Baudrate Read : " << servo->readBaudrate(id) << std::endl;

    // CW Limit
    std::cout << "CW Limit Write : " << std::endl;
    servo->writeLimitCWPosition(id, -135);
    std::cout << "CW Limit Read : " << servo->readLimitCWPosition(id) << std::endl;

    // CCW Limit
    std::cout << "CCW Limit Write : " << std::endl;
    servo->writeLimitCCWPosition(id, 135);
    std::cout << "CCW Limit Read : " << servo->readLimitCCWPosition(id) << std::endl;

    // Temperature Limit
    std::cout << "Temperature Limit Write : " << std::endl;
    servo->writeLimitTemperature(id, 70);
    std::cout << "Temperature Limit Read : " << servo->readLimitTemperature(id) << std::endl;

    // Current Limit
    std::cout << "Current Limit Write : " << std::endl;
    servo->writeLimitCurrent(id, 5000);
    std::cout << "Current Limit Read : " << servo->readLimitCurrent(id) << std::endl;

    // Drive Mode
    std::cout << "Drive Mode Write : " << std::endl;
    servo->writeDriveMode(id, 4);
    ...
    // Speed
    std::cout << "Speed Write : " << std::endl;
    servo->writeSpeed(id, 100);
    std::cout << "Speed Read : " << servo->readSpeed(id) << std::endl;

    // Accel Time
    std::cout << "Accel Time Write : " << std::endl;
    servo->writeAccelTime(id, 0.2);
    std::cout << "Accel Time Read : " << servo->readAccelTime(id) << std::endl;

    // Target Time
    std::cout << "Target Time Write : " << std::endl;
    servo->writeTargetTime(id, 0.1);
    std::cout << "Target Time Read : " << servo->readTargetTime(id) << std::endl;

    // Target Position
    std::cout << "Target Position Write : " << std::endl;
    servo->writeTargetPosition(id, 90);
    Sleep(1000);
    std::cout << "Target Position Write : " << std::endl;
    servo->writeTargetPosition(id, -33.3);
    Sleep(1000);
    ...
    std::cout << "Burst Position Write : " << std::endl;
    servo->burstWriteTargetPositions(idList, positionList, 1);

    servo->changeOperatingMode(true);

    std::cout << "Burst Position Read : " << std::endl;
    servo->burstReadPositions(idList, 1, burstReadCallback);
    //    std::cout << "Burst Position Read : " << std::endl; servo->readCurrentPosition(id,
    //    burstReadCallback);
    while (true)
        servo->spin();
}
...
//   ファイルを作成するか、[プロジェクト] > [既存の項目の追加] と移動して既存のコード
//   ファイルをプロジェクトに追加します
//   6. 後ほどこのプロジェクトを再び開く場合、[ファイル] > [開く] > [プロジェクト] と移動して .sln
//   ファイルを選択します
