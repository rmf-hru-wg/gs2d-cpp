...
class WindowsSerial
{
  ...
  public:
    ...
    int open(void)
    {
        handler = CreateFileA("\\\\.\\COM3", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING,
    ...
    int isConnected(void) { return connect_flag ? 1 : 0; }

    void close(void) { CloseHandle(handler); }
    ...
    int read(unsigned char *data, int size)
    {
        ...
        if (status.cbInQue > 0)
        {
            ...
            memset(data, 0, size);
            if (ReadFile(handler, data, num, &readbytes, NULL))
                return readbytes;
        }
    ...
    unsigned long long int time(void) { return GetTickCount(); }
};
} // namespace gs2d
